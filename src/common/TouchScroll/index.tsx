...
export default class TouchScroll extends React.Component<Props, State> {
    ...
    init() {
        this.children = this.props.children as React.ReactNode & { length: number }
        this.scrollContainer = this.containerRef!.current.firstElementChild as HTMLElement
        this.scrollItem = this.scrollContainer!.firstElementChild! as HTMLElement
        this.setWidth()
        this.scrollItemWidth = this.scrollItem.clientWidth
        this.scrollDistanceX = this.scrollContainer.children[0].clientWidth - this.scrollContainer.clientWidth
        if (this.paddingLeft !== 0) {
            this.maxScrollX = this.scrollDistanceX + this.paddingLeft
        } else if (this.marginRight !== 0) {
            this.maxScrollX = this.scrollDistanceX + 2 * this.marginRight - this.marginRight / 2
        } else {
            this.maxScrollX = this.scrollDistanceX
        }
        this.bindEvent()
    }
    getCSSAttribute(el: HTMLElement & { currentStyle: CSSStyleDeclaration }, attr: string) {
        if (el.currentStyle) {
    ...
    setWidth() {
        ...
        this.marginRight = parseInt(this.getCSSAttribute(this.scrollItem as any, 'margin-right'), 10)
        this.paddingLeft = parseInt(this.getCSSAttribute(this.scrollItem as any, 'padding-left'), 10)
        let totalWidth = 0
        for (let i: number = 0; i < this.scrollContainer.children.length; i++) {
            const child = this.scrollContainer.children[i] as HTMLElement
            const paddingL = parseInt(this.getCSSAttribute(child as any, 'padding-left'), 10)
            const paddingR = parseInt(this.getCSSAttribute(child as any, 'padding-right'), 10)
            child.style.width = this.wrapperWidth - this.marginRight - paddingL - paddingR - 10 + 'px'
            if (i === this.scrollContainer.children.length - 1 && (paddingL === 0 && paddingR === 0)) {
                child.style.width = this.wrapperWidth - this.marginRight + 'px'
            }
            totalWidth += child.clientWidth
        }
        this.scrollContainer.style.width = totalWidth + this.marginRight * this.scrollContainer.children.length + 'px'
    }
    bindEvent() {
    ...
    move(e: TouchEvent) {
        ...
        if (this.directionX < 0 && isTouchDistanceX) this.x += this.scrollItemWidth + this.marginRight
        if (this.directionX > 0 && isTouchDistanceX) this.x -= this.scrollItemWidth + this.marginRight
        ...
        if (
            this.lastTime - this.startTime < this.defaultTime && this.distanceXAbs >= this.defaultDistanceX - 30 ||
    ...
    resetPos() {
        const resetX = this.x >= 0 ? 0 : this.x <= this.scrollDistanceX ? this.scrollDistanceX : this.x
        if (resetX === 0) {
            this.x = 0
        }
    ...
    render() {
        return (
            <div className={ 'scroll_container' } ref={ this.containerRef } style={{ overflow: 'hidden' }} data-istransform={ 'transform' }>
                { this.props.children }
            </div>
        )
    }
