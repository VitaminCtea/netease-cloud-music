...
const Placeholder = styled.div`
    ...
    left: 0;
    width: 100%;
    height: ${(props: PlaceholderProps) =>
        typeof props.height === 'number' ? props.height + 'px' : '100%'};
    object-fit: ${(props: PlaceholderProps) =>
        props.objectFit ? props.objectFit : 'cover'};
    border-radius: ${(props: PlaceholderProps) =>
        props.borderRadius ? props.borderRadius : '8px'};
    background-color: ${(props: PlaceholderProps) =>
        props.theme ? props.theme.bgColor : 'cornflowerblue'};
`
...
let lazyLoadHandler = (scrollPort: HTMLElement) => {
    if (
        scrollPort.clientHeight + scrollPort.scrollTop >=
...
> {
    ...
    componentDidMount() {
        const component = ReactDom.findDOMNode(this) as HTMLElement
        this.scrollPort = document.getElementById('root')!.children[1]!
        ...
        if (this.props.overflow) {
            this.innerScrollPort = getTransformParent(component)
            if (this.innerScrollPort!.nodeName !== 'HTML') {
                isTransform = true
            } else {
                isTransform = false
                this.innerScrollPort = getScrollParent(component)
            }
        }

        if (this.props.scrollContainer) {
            if (isString(this.props.scrollContainer)) {
                this.innerScrollPort = document.querySelector(
        ...
        if (!finalLazyLoadHandler) {
            if (this.props.debounce !== undefined) {
                finalLazyLoadHandler = debounce(
                    lazyLoadHandler.bind(null, this.scrollPort as HTMLElement),
                ...
                finalLazyLoadHandler = throttle(
                    lazyLoadHandler.bind(null, this.scrollPort as HTMLElement),
                    typeof this.props.throttle === 'number'
                        ? this.props.debounce
                        : 300
                )
                delayType = 'throttle'
            } else {
                finalLazyLoadHandler = lazyLoadHandler.bind(
                    null,
                    this.scrollPort as HTMLElement
                )
            }
        }
        ...
        if (this.props.overflow) {
            if (
                this.innerScrollPort &&
                typeof this.innerScrollPort!.getAttribute === 'function'
            ) {
                const listenerCount =
                    1 + +this.innerScrollPort!.getAttribute(LISTEN_FLAG)!
                if (listenerCount === 1) {
                    if (isTransform) {
                        this.innerScrollPort!.addEventListener(
                            'touchmove',
                            finalLazyLoadHandler,
                            passiveEvent
                        )
                ...
                this.innerScrollPort!.setAttribute(
                    LISTEN_FLAG,
                    listenerCount + ''
                )
            }
